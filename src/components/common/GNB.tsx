...
function GNB({ nickname, profileImage }: GNBProps) {
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center w-full h-[80px] desktop:px-50 tablet:px-[30px] px-6 tablet:py-6 py-4">
                <Link href="/" >
                <img src={logo.src} alt="logo" className="hidden tablet:block" />
                <img src={symbolLogo.src} alt="logo" className="block tablet:hidden" />
                </Link>
                {!isLogin && (
                    <div className="flex items-center gap-[39px] text-14 text-gray-950">
                        <Link href="/auth/login">
                            <button className="hover:text-primary-500">로그인</button>
                        </Link>
                        <Link href="/auth/signup">
                            <button className="hover:text-primary-500">회원가입</button>
                        </Link>
                    </div>
                )}
                {isLogin && (
                        ...
                        <div className="relative">
                            ...
                            {isProfileOpen && (
                                        ...
                                        <ul className="w-full flex flex-col gap-[14px] justify-center text-16 text-gray-600">
                                            <li>
                                                <Link href="/main/profile" className="flex items-center justify-center py-[15px] hover:bg-primary-100 hover:text-gray-950 rounded-xl">로그아웃</Link>
                                            </li>
                                            <li>
                                                <Link href="/main/reservations" className="flex items-center justify-center py-[15px] hover:bg-primary-100 hover:text-gray-950 rounded-xl">마이페이지</Link>
                                            </li>
                                        </ul>
                                    </div>
                                )}
